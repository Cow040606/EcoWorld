                         ...
                         out float3 NormalOS_Out)
{
    ...
    float3 dispOS = mul((float3x3)GetWorldToObjectMatrix(), disp);

    PositionOS_Out = PositionOS + dispOS;

    NormalOS_Out = normalize(mul((float3x3)GetWorldToObjectMatrix(), normalWS));
}
