    ...
    public class FixKingDialogue
    {
        ...
        public static void FixNow()
        {
            try
            {
                ...
                NPC_DialogueTrigger prefabTrigger = prefab.GetComponentInChildren<NPC_DialogueTrigger>();
                ...
                foreach (var holder in holders)
                {
                    ...
                    else if (holder.NodeID == 31) // Reclaim find King
                    {
                        if (returnKing != null)
                            SetupPersistentCall(calls, bridge, "ThuHoiViecTuPlayer", returnKing);
                    }
                    else if (holder.NodeID == 34) // Timeline 4 event
                    {
                        if (prefabTrigger != null)
                            SetupPersistentCallVoid(calls, prefabTrigger, "ChayTimeline4");
                    }
                ...
                foreach (var sceneConv in sceneConvs)
                {
                    ...
                    if (sceneConv.gameObject.name.Contains("SM_Chr_King_01"))
                    {
                        ...
                        NPC_DialogueTrigger sceneTrigger = sceneConv.GetComponent<NPC_DialogueTrigger>();
                        GameObject tl04Obj = GameObject.Find("TIMELINE_04");
                        if (tl04Obj == null) tl04Obj = GameObject.Find("TIME");
                        ...
                        foreach (var sh in sceneHolders)
                        {
                            ...
                            if (sh.NodeID == 34 && sceneTrigger != null && tl04Obj != null)
                            {
                                SerializedObject soSh = new SerializedObject(sh);
                                soSh.Update();
                                SerializedProperty shCalls = soSh.FindProperty("Event.m_PersistentCalls.m_Calls");
                                SetupPersistentCallGameObject(shCalls, sceneTrigger, "ChayCutsceneCuThe", tl04Obj);
                                soSh.ApplyModifiedProperties();
                            }
                        }

                        if (tl04Obj != null && tl04Obj.activeSelf)
                        {
                            tl04Obj.SetActive(false);
                            EditorUtility.SetDirty(tl04Obj);
                        }
        ...
        private static QuestSO LoadQuestAsset(string guid)
        {
            ...
            return AssetDatabase.LoadAssetAtPath<QuestSO>(path);
                ...
                private static void SetupPersistentCallGameObject(SerializedProperty callsList, MonoBehaviour target, string methodName, GameObject argument)
        {
            ...
            args.FindPropertyRelative("m_ObjectArgumentAssemblyTypeName").stringValue = "UnityEngine.GameObject, UnityEngine";
            
            call.FindPropertyRelative("m_CallState").enumValueIndex = 2; // EditorAndRuntime
        }

        private static void SetupPersistentCallVoid(SerializedProperty callsList, MonoBehaviour target, string methodName)
        {
            callsList.ClearArray();
            callsList.InsertArrayElementAtIndex(0);
            SerializedProperty call = callsList.GetArrayElementAtIndex(0);
