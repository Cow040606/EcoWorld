...
public class Menu_Fusion : MonoBehaviour
{
    ...
    async void KetNoi(GameMode cheDo)
    {
        ...
        // 2. CHỐT CHẶN: Chỉ bắt lỗi nếu chơi Coop (Host/Client) mà quên nhập tên
        if (cheDo != GameMode.Single && string.IsNullOrEmpty(tenPhong))
        {
            ...
            return; // Đuổi về, không cho chạy tiếp
        }

        // 3. ĐẶC CÁCH CHƠI ĐƠN: Nếu chơi đơn mà chưa nhập tên, tự tạo tên ảo để lướt qua luôn
        if (cheDo == GameMode.Single && string.IsNullOrEmpty(tenPhong))
        {
            tenPhong = "PhongChoiDon_" + System.Guid.NewGuid().ToString().Substring(0, 5);
        }

        // 4. Bắt đầu kết nối và Load Scene
        ...
        await runner.StartGame(new StartGameArgs()
        {
            ...
            AuthValues = idRieng,
            Scene = SceneRef.FromIndex(1), // Đảm bảo Map 1 đang nằm ở vị trí số 1 trong Build Settings nhé!
