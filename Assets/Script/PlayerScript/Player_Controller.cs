...
public class Player_Controller : NetworkBehaviour, INetworkRunnerCallbacks
{
    ...
    // ========================================================
    // 🛠 ĐÂY LÀ HÀM ĐÃ ĐƯỢC SỬA LỖI CHẶT CÂY BẰNG HITBOX
    // ========================================================
    private void ThucHienXetVaChamChop()
    {
        // Tạo tâm quét va chạm (Hitbox) trước mặt nhân vật
        Vector3 hitboxCenter = transform.position + transform.forward * hitboxOffset + Vector3.up * 1f;

        // 1. CHẶT CÂY PREFAB (Tương tác với TreeScript)
        Collider[] hits = Physics.OverlapSphere(hitboxCenter, hitboxRadius, chopLayer);
        bool daTrungCay = false;

        foreach (var col in hits)
        {
            TreeScript cayMucTieu = col.GetComponentInParent<TreeScript>();
            if (cayMucTieu != null && cayMucTieu.IsActive)
            {
                // Trừ máu bằng sát thương cơ bản
                cayMucTieu.RPC_TakeDamage(baseDamage);
                daTrungCay = true;
                break; // Chém trúng 1 cây là dừng lại, không chém lan
            }
        }
        ...
        if (daTrungCay)
        {
            ...
            return; // Đã chém trúng cây Prefab thì kết thúc hàm luôn
        }

        // 2. CHẶT CÂY TERRAIN (Giữ nguyên logic cũ phòng trường hợp bạn dùng hệ sinh thái cũ)
        LayerMask maskDung = (chopLayer.value != 0) ? chopLayer : Physics.DefaultRaycastLayers;
        Terrain hitTerrain = null;

        // Bắn 1 tia ngắn xuống dưới để chắc chắn lấy đúng Terrain
    ...
    private void OnToolChanged()
    {
        ...
        if (idDangCam > 0 && InventoryManager.instance != null)
        {
            ...
            {
                // 1. Sinh ra vũ khí và làm con trực tiếp của viTriCamVuKhi
